...
key = '1234567890123456' # create new & store somewhere safe
    ...
    for i in range(len(tmp1)):
        if i in star_index_values:
            if i == star_indices["dob"]:
                x, y = test_and_encrypt(tmp1[i],tmp2[i])
...
def star_similarities(d, star_indices,index_file_id):
    ...
    for p in d:
        for i in range(1, len(d[p])):
            ...
            d[p][0][title.index("sex") - 1] = encrypt2(key, d[p][0][title.index("sex") - 1])
            d[p][0][title.index("race") - 1] = encrypt2(key, d[p][0][title.index("race") - 1])
            d[p][i][title.index("sex") - 1] = encrypt2(key,d[p][i][title.index("sex") - 1])
            d[p][i][title.index("race") - 1] = encrypt2(key,d[p][i][title.index("race") - 1])
...
write_data(data, "./data_intermediate/all_starred_race_enc.csv",title_array)